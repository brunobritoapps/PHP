...
<body>
    ...
    <div id="right-panel" class="right-panel">
		...
		<div class="col-lg-12">
			<table class="table table-bordered" style="font-size:80%;">
			  ...
			  <tbody>
				<tr>
				  <th bgcolor="Red"></th>
				  <td>Reservado - Frete Retorno</td>
				  <th bgcolor="yellow"></th>
				  <td>Reservado - Manutenção</td>
				  <th bgcolor="blue"></th>
				  <td>Reservado - Retorno Vazio</td>
				</tr>
			  </tbody>
			</table>
			</div>
			...
			Busca: <input type="text" name="pesq" id="pesq">
            <button type="submit" name="btpesq" id="pesq" class="btn btn-warning">Pesquisar</button>
			</form>
		...
		if(isset($_POST['btpesq']))
			...
			$string = $dados["res_solicita"].$dados["res_veic"].$dados["res_placa1"];
			$exist 	= preg_match ("/".$_POST["pesq"]."/", $string);
			
					if($dados['res_embarca'] <> 'MANUTENCAO' AND $dados['res_embarca'] <> 'RETORNO VAZIO') { 					
				
			?>	
            <button style=<?php if( !$exist ){ echo "display:none;";} ?> style="font-size:80%;" onclick="myFunction('Demo<?php echo $nCont ?>')" class="w3-button w3-block w3-red w3-left-align aprova">FRETE RETORNO | SOLICITANTE: <?php echo $dados["res_solicita"]; ?> | VEICULO: <?php echo $dados["res_veic"]; ?> | PLACA MOTOR: <?php echo $dados["res_placa1"]; ?></button>
			...
			-->				
				  ...
				  <form action="aprova.php" method="POST" enctype="multipart/form-data" class="form-horizontal"><br>
                    ...
                    <div class="col-12 col-md-9">
						<textarea name="aprovaobs" id="aprovaobs" required=""  rows="8" placeholder="" class="form-control"></textarea>
				...
				?>
				<button style=<?php if( !$exist ){ echo "display:none;";} ?> style="font-size:80%;" onclick="myFunction('Demo<?php echo $nCont ?>')" class="w3-button w3-block w3-<?php if ( $dados["res_embarca"] == "RETORNO VAZIO" ){echo "blue";}else if( $dados["res_embarca"] == "MANUTENCAO"){ echo "yellow";} ?> w3-left-align"><?php if ( $dados["res_embarca"] == "RETORNO VAZIO" ){echo "RETORNO VAZIO | ";}else if( $dados["res_embarca"] == "MANUTENCAO"){ echo "MANUTENÇÃO   | ";} ?> SOLICITANTE: <?php echo $dados["res_solicita"]; ?> | VEICULO: <?php echo $dados["res_veic"]; ?> | PLACA MOTOR: <?php echo $dados["res_placa1"]; ?></button>
				...
				<div class="animated fadeIn">
						...
						<div class="row form-group">
							<form action="aprova.php" method="POST" enctype="multipart/form-data" class="form-horizontal">
								...
								<div class="col-12 col-md-9"><textarea name="aprovaobs" id="aprovaobs" rows="8" placeholder=""  required=""  class="form-control"> <?php echo $dados["res_obsest"]; ?></textarea></div><br><br>
    ...
    <!-- Right Panel -->
	<?php
    if (isset($_SESSION['res_solicita'])) {
